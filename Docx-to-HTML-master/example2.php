...
//        foreach ($files as $filename) {
        foreach ($this->indexes as $filename => $index) {
            ...
            if ($ext === 'wmf') {
                ...
                $newName = str_replace($ext, '', $filename) . "png";
                file_put_contents($this->savepath . '/' . $newName, $image);
                $this->indexes[$newName] = $index;
                $imgNew= new Imagick($dir.'/'.$newName);
                $imgBuff = $imgNew->getimageblob();
                $this->files[]=base64_encode($imgBuff);
                unset($this->indexes[$filename]);
            }
        }
        var_dump($this->files);
    }
...
$DocxImages = new DocxImages("sample.docx");
